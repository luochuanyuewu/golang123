<template>
  <div class="book-box">
    <div class="book-container">
      <div
        class="book-tree-box"
        :class="{ 'book-tree-box-fixed': chapterTreeFixed }"
        :style="{ height: chapterTreeHeight }"
      >
      ...
      <div
        class="book-content-box"
        :class="{ 'book-content-box-expand': chapterTreeFixed }"
      >
        ...
        <div
          ...
          v-html="chapter.htmlContent"
        ></div>
        <div class="book-prev-next">
          <div v-if="prevChapter" class="book-prev">
            上一篇:
            <a :href="`/book/${book.id}?chapterID=${prevChapter.id}`">{{
              prevChapter.title
            }}</a>
          </div>
          <div v-if="nextChapter" class="book-next">
            下一篇:
            <a :href="`/book/${book.id}?chapterID=${nextChapter.id}`">{{
              nextChapter.title
            }}</a>
          </div>
        </div>
...
export default {
  ...
  head() {
    return {
      title:
        htmlUtil.entity2HTML(this.book.name) +
        ' - ' +
        htmlUtil.entity2HTML(this.chapter.name),
      link: [
            ...
            [
              ...
              h(
                'span',
                {
                  style: {
                    color:
                      this.chapter && data.id === this.chapter.id
                        ? '#348eed'
                        : '#333'
                  }
                },
