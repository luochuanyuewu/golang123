    ...
    <Col :span="16" class="admin-tag-form">
      ...
      >
        ...
        >
          ...
          <Form-item>
            <Button type="primary" @click="handleEditSubmit('editDynamic')"
              >确定</Button
            >
        ...
        <span>
          <span
            ...
            @click="edit(item.id, index, item.name)"
            >编辑</span
          >
          <span class="admin-status-tag" @click="change(item.id, index)">{{
            item.status == 1 ? '禁用' : '启用'
          }}</span>
        </span>
      </Row>
      <Form ref="formDynamic" :model="formDynamic" class="admin-tag-form">
        ...
        <Form-item v-if="editStatus === 'common'">
          <Row>
            <Col span="12">
              <Button type="dashed" long @click="handleAdd" icon="plus-round"
                >新增</Button
              >
            </Col>
          </Row>
  ...
  methods: {
    ...
    change(id, index) {
      ...
      this.$Modal.confirm({
        title: '提醒',
        content: `是否${
          self.list[index].status === 2 ? '启用' : '禁用'
        }该标签?`,
        loading: true,
        onOk: () => {
