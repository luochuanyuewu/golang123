    ...
    <div>
      <div class="home-categoties-box">
        <a
          ...
          :class="{ 'categoties-select': !status }"
          >全部</a
        >
        <a
          class="categoties-item"
          :href="'/vote?status=' + voteStatus.VOTE_UNDERWAY"
          :class="{ 'categoties-select': status == voteStatus.VOTE_UNDERWAY }"
          >投票中</a
        >
        <a
          class="categoties-item"
          :href="'/vote?status=' + voteStatus.VOTE_OVER"
          :class="{ 'categoties-select': status == voteStatus.VOTE_OVER }"
          >已结束</a
        >
      </div>
      <div class="home-articles-box">
        <div v-for="vote in votes" class="articles-cell">
          <a
            class="user-icon-box"
            :href="`/user/${vote.user.id}`"
            target="_blank"
            ><img :src="vote.user.avatarURL" alt=""
          /></a>
          <span class="home-tip-container">
            <Tooltip
              :content="`回复数${vote.commentCount}　浏览数${vote.browseCount}`"
          ...
          <a :href="'/vote/' + vote.id" class="home-articles-title">{{
            vote.name | entity2HTML
          }}</a>
          <p class="articles-res-time">{{ vote.createdAt | getReplyTime }}</p>
          <a
            :href="`/user/${vote.lastUser.id}`"
            target="_blank"
            class="user-small-icon-box"
            ><img :src="vote.lastUser.avatarURL" alt=""
          /></a>
        </div>
      </div>
      <baidu-banner900x110 />
