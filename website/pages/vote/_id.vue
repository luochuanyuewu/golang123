  ...
  <div>
      ...
      <div>
        <div class="vote-detail-box">
          ...
          <div class="home-vote-box">
            <div
              class="golang123-editor golang123-richtxt"
              v-html="vote.htmlContent"
            ></div>
            <div class="" style="margin-bottom: 20px;">
              <span v-for="item in vote.voteItems">
                <Button
                  type="primary"
                  class="vote-item"
                  @click="onVoteSubmit(item.id)"
                  >支持<span class="vote-item-label">{{ item.name }}</span
                  ><span class="vote-item-label">{{ item.count }}</span></Button
                >
              </span>
            </div>
            <div
              v-if="!isEnd"
              style="margin-bottom: 20px;font-size: 14px;font-weight: 700;"
            >
              {{ endAtStr }}&nbsp;&nbsp;前可投票
            </div>
        ...
        <div class="golang-cell comment-box">
          <div class="title total-reply-count">
            {{ vote.commentCount > 0 ? vote.commentCount : '暂无' }}回复
          </div>
          <div class="comment-content">
            <template v-if="vote.commentCount">
              <div
                :id="`reply-${item.id}`"
                class="comment-item"
                v-for="(item, index) in vote.comments"
              >
                ...
                <div class="reply-user-box">
                  ...
                  <div v-if="item.parentID" class="parent-comment">
                    <template v-if="item.parents && item.parents.length">
                      ...
                      <span
                        ><a
                          :href="`/user/${item.parents[0].user.id}`"
                          target="_blank"
                          ><img :src="item.parents[0].user.avatarURL"/></a
                      ></span>
                      <span
                        ><a
                          :href="`/user/${item.parents[0].user.id}`"
                          target="_blank"
                          class="parent-comment-user"
                          >{{ item.parents[0].user.name }}</a
                        ></span
                      >
                      <span
                        ><a
                          :href="`/vote/${vote.id}#reply-${item.parents[0].id}`"
                          >{{ floorMap[item.parents[0].id] }}楼</a
                        ></span
                      >
                      <span>回复</span>
                    </template>
                  ...
                  <div v-if="item.replyVisible || item.editReplyVisible">
                    <div>
                      <md-editor
                        ...
                        @change="onContentChage"
                      />
                    </div>
                    <Row>
                      <Button @click="onEditOrSubmitReply(item)" type="primary"
                        >保存</Button
                      >
                      <Button
                        @click="cancelReply"
            ...
            <p class="not-signin" v-if="!vote.commentCount && user">
              暂时还没有人回复过这个投票
            </p>
            <p class="not-signin" v-if="!vote.commentCount && !user">
              暂时还没有人回复过这个投票,&nbsp;要回复投票, 请先&nbsp;<a
                @click="onSignin"
                >登录</a
              >&nbsp;或&nbsp;<a href="/signup">注册</a>
            </p>
            <p
              class="not-signin not-signin-border"
              v-if="vote.commentCount && !user"
            >
              要回复投票, 请先&nbsp;<a @click="onSignin">登录</a
              >&nbsp;或&nbsp;<a href="/signup">注册</a>
            </p>
          </div>
        </div>
    ...
    >
      <Row
        ...
        align="middle"
        v-for="(item, index) in collectDirList"
        key="index"
      >
        <div>
          <a
            :href="`/user/collect/${user.id}?folder=${item.id}`"
            target="_blank"
            class="collects-item-label"
            >{{ item.name }}</a
          >
          <p class="collects-item-num">
            {{ (item.collects && item.collects.length) || 0 }}条内容
          </p>
        </div>
        <Button
          v-if="item.hasCollect"
          class="info-button"
          style="width: 80px"
          disabled="disabled"
          >已收藏</Button
        >
        <Button
          v-else-if="!alreadyCollect"
          class="info-button"
          style="width: 80px"
          @click="createCollect(item.id)"
          >收藏</Button
        >
      </Row>
      <Button
        type="primary"
        size="large"
        class="collect-dir-btn"
        @click="createCollectDir"
        >创建收藏夹</Button
      >
      <div slot="footer"></div>
    </Modal>
    <Modal
      ...
      @on-cancel="cancel"
    >
      <Form ref="CollectDir" :model="collectData" :rules="collectRule">
      ...
      <Row type="flex" justify="space-between">
        ...
        <Button type="primary" style="width:48%" @click="submitCollectDir"
          >确认创建</Button
        >
      </Row>
      <div slot="footer"></div>
...
export default {
        ...
        if (arr[1]) {
          ...
          collectDirList.map((item) => {
            ...
            item.collects.map((items) => {
              if (
                items.sourceID === parseInt(context.params.id) &&
                items.sourceName === 'collect_source_vote'
              ) {
  ...
  mounted() {
    if (
      ...
      window.location.search.indexOf('pushLink=1') >= 0
    ) {
  ...
  head() {
    return {
      ...
      script: [{ src: '/javascripts/highlight/highlight.min.js' }]
    }
  },
  filters: {
