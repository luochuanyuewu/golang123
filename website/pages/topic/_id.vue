      ...
      <div>
        <div class="topic-detail-box">
          <div class="detail-title-box">
            <div class="article-detail-title">
              <h1>{{ article.name | entity2HTML }}</h1>
            </div>
        ...
        <div class="golang-cell comment-box">
          <div class="title total-reply-count">
            {{ article.commentCount > 0 ? article.commentCount : '暂无' }}回复
          </div>
          <div class="comment-content">
                  ...
                  <div v-if="item.parentID" class="parent-comment">
                    <template v-if="item.parents && item.parents.length">
                      ...
                      <span
                        ><a
                          :href="
                            `/topic/${article.id}#reply-${item.parents[0].id}`
                          "
            ...
            <p class="not-signin" v-if="!article.commentCount && user">
              暂时还没有人回复过这个话题
            </p>
            <p class="not-signin" v-if="!article.commentCount && !user">
              暂时还没有人回复过这个话题,&nbsp;要回复话题, 请先&nbsp;<a
                @click="onSignin"
                >登录</a
            ...
            <p
              ...
              v-if="article.commentCount && !user"
            >
              要回复话题, 请先&nbsp;<a @click="onSignin">登录</a
              >&nbsp;或&nbsp;<a href="/signup">注册</a>
            </p>
          </div>
        </div>
            ...
            collectDirList.map((item) => {
              ...
              item.collects.map((items) => {
                if (
                  ...
                  items.sourceName === 'collect_source_article'
                ) {
              ...
              .then((res) => {
                if (res.errNo === ErrorCode.SUCCESS) {
                  ...
                  collectDirList.map((item) => {
                    ...
                    item.collects.map((items) => {
                      if (
                        items.sourceID === parseInt(self.$route.params.id) &&
                        items.sourceName === 'collect_source_article'
                      ) {
                        item.hasCollect = false
                      }
