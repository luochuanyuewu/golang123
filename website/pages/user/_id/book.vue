  ...
  <div class="articles-container">
    ...
    <template v-if="books.length > 0">
      <div
        ...
        class="articles-item"
        :class="{ 'articles-item-no': index === 0 }"
      >
        ...
        <div class="book-intro">
          <h2 class="book-title">
            <a :href="`/book/${book.id}`" target="_blank">{{ book.name }}</a>
          </h2>
          ...
          <div v-if="isAuthor" class="book-actions">
            <Dropdown @on-click="onClickBookDropdown">
              ...
              <DropdownMenu slot="list">
                <DropdownItem
                  v-if="book.status === 'book_unpublish'"
                  :name="`book_unpublish-${book.id}`"
                  >发布</DropdownItem
                >
                <DropdownItem
                  v-else-if="book.status === 'book_verifying'"
                  :name="`book_verifying-${book.id}`"
                  disabled
                  >审核中</DropdownItem
                >
                <DropdownItem
                  v-else-if="book.status === 'book_verify_success'"
                  :name="`book_verify_success-${book.id}`"
                  disabled
                  >已发布</DropdownItem
                >
                <DropdownItem
                  v-else-if="book.status === 'book_verify_fail'"
                  :name="`book_verify_fail-${book.id}`"
                  disabled
                  >未通过审核</DropdownItem
                >
                <DropdownItem divided :name="`book_edit-${book.id}`"
                  ><a :href="`/book/edit/${book.id}`">编辑图书</a></DropdownItem
                >
                <DropdownItem :name="`book_chapter_edit-${book.id}`"
                  ><a :href="`/book/edit/chapter/${book.id}`"
                    >编辑章节</a
                  ></DropdownItem
                >
                <DropdownItem :name="`book_delete-${book.id}`"
                  >删除</DropdownItem
                >
              </DropdownMenu>
            </Dropdown>
