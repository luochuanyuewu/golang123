<template>
  <div class="articles-container">
    <div class="article-top">
      <div>
        {{ user && user.id == this.currentId ? '我' : sex ? '她' : '他' }}的回复
      </div>
    </div>
    <template v-if="comments.length > 0">
      ...
      >
        <h2 class="articles-title">
          <a
            v-if="!comment.noSource"
            :href="
              comment.voteID
                ? `/vote/${comment.voteID}#reply-${comment.id}`
                : `/topic/${comment.articleID}#reply-${comment.id}`
            "
            target="_blank"
            >{{ comment.voteName ? comment.voteName : comment.articleName }}</a
          >
          <span v-else class="reply-no-source">{{
            comment.sourceName === 'vote'
              ? '原投票已被作者删除'
              : '原话题已被作者删除'
          }}</span>
        </h2>
