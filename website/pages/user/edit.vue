  ...
  <div>
    <div class="golang-home-body">
      <div class="golang-main-top">
        <div
          class="mine-img-box"
          :style="{ 'background-image': 'url(' + userInfo.coverURL + ')' }"
        ></div>
        <div class="mine-edit-container">
          ...
          <p class="mine-info-line mine-info-name">
            {{ user.name }}
            <a class="home-link-box no-underline" :href="`/user/${user.id}`"
              >返回我的首页&nbsp&nbsp<Icon type="chevron-right"></Icon
            ></a>
          </p>
          ...
          <div class="mine-info-line mine-info-edit">
            ...
            <span class="mine-info-value" v-if="editIndex[1] === 0">
              ...
              <span
                class="edit-action"
                :style="!userInfo.signature ? 'display: inline' : ''"
                @click="editItem('signature', 1)"
              >
                <i class="ivu-icon ivu-icon-edit"></i>
                修改
              </span>
            ...
            <div v-else class="edit-item">
              ...
              <Row class="button-box">
                <Button
                  type="primary"
                  class="button-seq"
                  @click="submit('signature', 1)"
                  >保存</Button
                >
                <Button type="ghost" @click="close('signature', 1)"
                  >取消</Button
                >
              </Row>
            </div>
          ...
          <div class="mine-info-line mine-info-edit">
            ...
            <span class="mine-info-value" v-if="editIndex[2] === 0"
              >{{ userInfo.location }}
              <span
                class="edit-action"
                :style="!userInfo.location ? 'display: inline' : ''"
                @click="editItem('location', 2)"
              >
                <i class="ivu-icon ivu-icon-edit"></i>
                修改
              </span>
          ...
          <div class="mine-info-line mine-info-edit">
            ...
            <div v-else class="edit-item">
              <Input
                v-model="formCustom.school.name"
                placeholder="学校或教育机构名"
                class="button-seq"
                style="width: 300px"
              ></Input>
              <Input
                v-model="formCustom.school.speciality"
                placeholder="专业方向"
                class="button-seq"
                style="width: 300px"
              ></Input>
              <Button type="primary" class="button-seq" @click="addSchool"
                >保存</Button
              >
              <Button type="ghost" @click="close('school', 4)">取消</Button>
            </div>
            <p class="schools-item" v-for="(item, index) in userInfo.schools">
              {{ item.name }}&nbsp{{
                item.speciality ? '•&nbsp' + item.speciality : ''
              }}
              <i
                class="ivu-icon ivu-icon-close schools-delete"
          ...
          <div class="mine-info-line mine-info-edit">
            ...
            <div v-else class="edit-item">
              <Input
                v-model="formCustom.career.company"
                placeholder="公司或组织名称"
                class="button-seq"
                style="width: 300px"
              ></Input>
              <Input
                v-model="formCustom.career.title"
                placeholder="你的职位"
                class="button-seq"
                style="width: 300px"
              ></Input>
              <Button type="primary" class="button-seq" @click="addCareer"
                >保存</Button
              >
              <Button type="ghost" @click="close('career', 5)">取消</Button>
            </div>
            <p class="schools-item" v-for="(item, index) in userInfo.careers">
              {{ item.company }}&nbsp{{ '•&nbsp' + item.title }}
              <i
                class="ivu-icon ivu-icon-close schools-delete"
                @click="deleteCareer(item.id)"
              ></i>
            </p>
          </div>
          <div class="mine-info-line mine-info-edit">
            ...
            <span class="mine-info-value" v-if="editIndex[3] === 0">
              ...
              <span
                class="edit-action"
                :style="!userInfo.introduce ? 'display: inline' : ''"
                @click="editItem('introduce', 3)"
              >
                <i class="ivu-icon ivu-icon-edit"></i>
                修改
              </span>
            ...
            <div v-else class="edit-item">
              ...
              <Row class="button-box">
                <Button
                  type="primary"
                  class="button-seq"
                  @click="submit('introduce', 3)"
                  >保存</Button
                >
                <Button type="ghost" @click="close('introduce', 3)"
                  >取消</Button
                >
              </Row>
            </div>
...
export default {
        ...
        onOk() {
          request
            ...
            .then((res) => {
              if (res.errNo === ErrorCode.SUCCESS) {
                let id = res.data.id
                self.userInfo.schools = self.userInfo.schools.filter(
                  (item) => item.id !== id
                )
        ...
        onOk() {
          request
            ...
            .then((res) => {
              if (res.errNo === ErrorCode.SUCCESS) {
                let id = res.data.id
                self.userInfo.careers = self.userInfo.careers.filter(
                  (item) => item.id !== id
                )
                self.$Message.success({
                  duration: config.messageDuration,
  ...
  head() {
    return {
      title: '个人信息',
      link: [{ rel: 'stylesheet', href: '/styles/croppie/croppie.min.css' }],
