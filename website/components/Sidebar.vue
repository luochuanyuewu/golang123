  ...
  <div class="common-body-right">
    <template v-if="!user && userLoginVisible">
      <div class="golang-cell golang-user-info">
        ...
        <p>
          您可以<a
            class="golang-user-info-action sidebar-user-signin"
            @click="onSignin"
            >登录</a
          >或<a
            class="golang-user-info-action sidebar-user-signup"
            href="/signup"
            >注册</a
          >
        </p>
    ...
    <div v-if="user && publishTopicVisible" class="golang-user-publish">
      <a href="/topic/create"
        ><button class="signup-button ivu-btn ivu-btn-primary ivu-btn-large">
          发布话题
        </button></a
      >
    </div>
    ...
    <div v-if="topicAuthor" class="golang-cell">
      ...
      <div class="golang-user-info" style="padding-top:16px;">
        <div>
          <a
            :href="'/user/' + topicAuthor.id"
            target="_blank"
            class="golang-sidebar-icon-box"
          >
            <img
              class="golang-user-info-icon"
              :src="topicAuthor.avatarURL"
              alt=""
            />
          </a>
          <span class="golang-sidebar-info-box">
            <a :href="'/user/' + topicAuthor.id" target="_blank">{{
              topicAuthor.name
            }}</a>
          </span>
        </div>
        ...
        <div class="golang-user-line author-info-text text-italic">
          {{ topicAuthor.signature || '这家伙很懒，什么个性签名都没有留下' }}
        </div>
      </div>
    </div>
    <div
      v-if="authorRecentArticles && authorRecentArticles.length"
      class="golang-cell"
    >
      <div class="title">作者近期话题</div>
      <ul>
        <li v-for="topic in authorRecentArticles" class="golang-cell-item">
          <a
            :href="`/topic/${topic.id}`"
            target="_blank"
            class="sidebar-articles-title"
            >{{ topic.name }}</a
          >
        </li>
      </ul>
    ...
    <baidu-ad250x250 />
    <div
      v-if="maxBrowseVisible && maxBrowseArticles && maxBrowseArticles.length"
      class="golang-cell"
    >
      <div class="title">热门话题</div>
      <ul>
        <li class="golang-cell-item" v-for="item in maxBrowseArticles">
          <a
            :href="`/topic/${item.id}`"
            target="_blank"
            class="sidebar-articles-title"
    ...
    <div
      v-if="
        maxCommentVisible && maxCommentArticles && maxCommentArticles.length
      "
      class="golang-cell"
    >
      <div class="title">回复最多的话题</div>
      <ul>
        <li class="golang-cell-item" v-for="item in maxCommentArticles">
          <a
            :href="`/topic/${item.id}`"
            target="_blank"
            class="sidebar-articles-title"
    ...
    <div v-if="votesMaxBrowse && votesMaxBrowse.length" class="golang-cell">
      ...
      <ul>
        <li class="golang-cell-item" v-for="item in votesMaxBrowse">
          <a :href="`/vote/${item.id}`" class="sidebar-articles-title">{{
            item.name
          }}</a>
        </li>
      </ul>
    ...
    <div v-if="votesMaxComment && votesMaxComment.length" class="golang-cell">
      ...
      <ul>
        <li class="golang-cell-item" v-for="item in votesMaxComment">
          <a :href="`/vote/${item.id}`" class="sidebar-articles-title">{{
            item.name
          }}</a>
        </li>
      ...
      <ul style="text-align: center;">
        ...
        <li class="golang-cell-item">
          <a
            ...
            ><img
              style="width: 150px;"
              src="https://dn-phphub.qbox.me/assets/images/friends/ruby-china.png"
          /></a>
        </li>
      </ul>
    </div>
