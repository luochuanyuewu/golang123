...
export default {
  ...
  data() {
    return {
      formValidate: {
        ...
        date:
          (this.vote && this.vote.endAt && new Date(this.vote.endAt)) || null,
        content: (this.vote && this.vote.content) || '',
        items: (this.vote &&
          this.vote.voteItems.map((item) => {
            return {
              id: item.id,
              value: item.name
            }
  ...
  methods: {
    ...
    addItem(index) {
      ...
      let func = this.formValidate.items[index].id
        ? request.editVoteItem
        : request.addVoteItem
      func({
        body: body
      })
